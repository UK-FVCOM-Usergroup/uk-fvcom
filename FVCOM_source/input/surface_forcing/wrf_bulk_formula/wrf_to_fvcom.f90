...
!---------------------------array of forcing variables-------------------
	...
	real,               allocatable, dimension(:,:,:)   :: rh_gen                      ! Relative humidity generated by QV
        real,               allocatable, dimension(:,:,:)   :: air_temp                    ! 2m air temperature
...
!------------------------------------------------------------------------------
	...
	allocate(pressure_slp(west_east,south_north,time_index))
        ...
        allocate(air_temp(west_east,south_north,time_index))
...
!---------Use TSK to fill the SST data for the land (where SST is set as -1)
	...
	urxx=sqrt(u10(i,j,k,1)**2+v10(i,j,k,1)**2)	! wind speed	
        ...
        air_temp(i,j,k)=taxx                            ! save this time step's air temperature
...
!--Set up static dimensions
       ...
       rcall = nf90_def_var(nc_ofid,"air_temperature",nf90_float,force3m,air_temp_id)
...
!!---------write data for forcing file---------------------------------------
      ...
      rcall =nf90_put_var(nc_ofid,air_temp_id,air_temp)
      vstart(1)=1
      vstart(2)=1
      vcount(1)=19
      vcount(2)=ntimes
